...
export function BecomeLienWizard({ permanentIdentity, freeGeneration }: { permanentIdentity: PermanentIdentity | null; freeGeneration: boolean }) {
  ...
  async function startCheckout() {
    setCheckoutLoading(true); setStatus(`Opening secure ${edition === "holographic" ? "$7" : "$3"} Stripe checkout.`); void track("checkout_started");
  ...
  return <section className="mx-auto grid max-w-6xl gap-5 px-4 pb-12 md:px-8">
    ...
    {step===4 && <HudPanel title="Step 4 · Choose Your Role" accent="#39FF14"><p className="mb-5 text-sm leading-6 text-zinc-300">Role means who you are and how you contribute. Season One roles do not provide gameplay advantages.</p><div className="grid gap-4 md:grid-cols-2">{seasonOneRoles.map(item=>{const p=roleProfiles[item];return <button type="button" aria-pressed={role===item} className={`role-choice ${role===item?"selected":""}`} onClick={()=>{setRole(item);void track("role_selected")}} key={item}><span className="role-choice__emblem">{p.emblemGlyph}</span><strong>{item}</strong><p>{p.purpose}</p><small>{p.traits.join(" · ")}</small></button>})}</div><div className="mt-5 border-l-2 border-lime-300 p-4"><strong className="font-display uppercase text-lime-200">{selectedRole.title}</strong><p className="mt-2 text-sm text-zinc-300">{selectedRole.charge}</p></div><button className="primary-cta mt-5" onClick={()=>setStep(5)}>Continue</button></HudPanel>}
