...
export async function POST(request: Request) {
  ...
  const prompt = [
    `Transform ${sanitizeUserText(parsed.data.humanName)} into a canonical DonLien species portrait.`,
    "Preserve recognizable identity and general pose.",
    "Use realistic alien-green skin, glossy black almond-shaped eyes, blonde presidential side-swept hair.",
    `Role-specific uniform: ${parsed.data.role}.`,
    "If a tie exists, make it match the outfit with vertical glowing green DONLIEN letters.",
    "No earrings. No random jewelry. Premium photorealism.",
  ].join(" ");
