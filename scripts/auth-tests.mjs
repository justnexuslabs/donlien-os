...
const files = [
  ...
  "app/api/lien/auth/route.ts",
  "components/MissionControl.tsx",
  "lib/mission-control.ts",
  "app/api/lien/profile/route.ts",
  "lib/content.ts",
...
const checks = [
  ...
  [content["app/mission-control/page.tsx"].includes("readLienSessionDetails"), "Mission Control requires a signed LIEN session"],
  [content["app/mission-control/page.tsx"].includes('redirect("/lien-id?next=/mission-control")'), "unauthenticated Mission Control redirects to Telegram connection"],
  [content["app/api/lien/auth/route.ts"].includes('url.searchParams.get("next") === "/mission-control"'), "Telegram return destination is allowlisted"],
  [content["components/MissionControl.tsx"].includes('fetch("/api/lien/profile"'), "Mission Control refreshes the authoritative server profile"],
  [content["lib/mission-control.ts"].includes('profile.gameStats?.["signal-lock"]'), "Mission progress derives from verified game statistics"],
  [!content["lib/mission-control.ts"].includes("fetch("), "Mission definitions cannot claim rewards from the browser"],
  [content["app/api/lien/profile/route.ts"].includes("missionControl: data.missionControl"), "rank and GLB activity remain server-sourced"],
